...
def sort_by_color(c_1):
    return str(clrs.to_hex(c_1.get_facecolor(), keep_alpha=False))
...
def main(fname, fout,fig_extns):
  ...
  # Reset matplotlib settings;
    ...
    for ax in g.axes.flat:
        p += ax.patches
    colors = set([clrs.to_hex(x.get_facecolor(), keep_alpha=False) for x in p])
    
    hatches_dict = {}
    for h,c in zip(hatches,colors):
        hatches_dict[c] = h
    colors_ins = set()
    legend_patches = []
    for ax in g.axes:
        for axis in ax:
            ...
            h = []
            for i in range(0,len(patches)//len(gpus)):
                h += hatches[i] * len(gpus)
            k = 0
     
            for patch in patches:
                patch.set_hatch(hatches_dict[clrs.to_hex(patch.get_facecolor(), keep_alpha=False)])
                if not ((str(clrs.to_hex(patch.get_facecolor(), keep_alpha=False))) in colors_ins):
                    colors_ins.add(str(clrs.to_hex(patch.get_facecolor(), keep_alpha=False)))
                    legend_patches.append(patch)
                    print(colors_ins)
                #if (k % len(gpus)) == 0:
                #    legend_patches.append(patch)
                k = k + 1
    print(len(legend_patches))
    list.sort(legend_patches, key=sort_by_color, reverse=True)
    sns.move_legend(g, "lower center", handles=legend_patches, bbox_to_anchor=(.46, 1), ncol=int(len(functions)/3), title=None, frameon=False)
