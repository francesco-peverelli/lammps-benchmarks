...
def main(fname, fout, fig_extns):
    ...
    for ax in g.axes:
        for axis in ax:
            ...
            for i in range(0,len(functions)):
                h += hatches[i] * len(procs)
            #h = hatches[0:len(procs)] * len(functions)
            k = 0
            for patch, hatch in zip(patches, h):
                patch.set_hatch(hatch)
                if (k % len(procs)) == 0:
    ...
    sns.move_legend(g, "lower center", handles=legend_patches, bbox_to_anchor=(.46, 1), ncol=len(functions), title=None, frameon=False)
