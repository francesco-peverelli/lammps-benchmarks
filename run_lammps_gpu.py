...
for gpu,mpi in gpu_mpi_dict[problem_idx].items():
    for k in range(0,10):
        bench_cmd="'mpiexec -np " + str(mpi) + " env OMP_NUM_THREADS=1 ../bin/lmp_intel_cpu_intelmpi-single -in " + bench_in + " -sf gpu -pk gpu " + str(gpu) + "'"
        tag = bench_in + "-single_" + str(gpu) + "g_" + str(mpi) + "n_"+ sys.argv[2]
